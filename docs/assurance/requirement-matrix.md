...
| Kafka, schema registry, CloudEvents, durable publication, poison records, and reconciliation | Partial | Current Kafka package and specialist evidence covers the supported local and real-broker contracts and is consumed without rerunning content-identical campaigns. | Complete managed broker, schema-registry, partition, leader-loss, and recovery composition beyond the package-owned support boundary. Operational-assurance owner. |
...
| Kafka rebalance, duplicates, reorder, partitions, leader loss, poison records, acknowledgement ambiguity, and registry outage | Partial | Current specialist evidence proves the package-owned broker, rebalance, duplicate, settlement, poison-record, and ambiguity contracts. | Complete managed partition, leader-loss, prolonged outage, and registry-recovery composition beyond the package-owned support boundary. Operational-assurance owner. |
...
| Dependency-ordered `v1.0.0` releases, clean external install, upgrade, downgrade, deprecation, and removal | Partial | [`OA-RELEASE-CONSUMER-LOCAL`](evidence/OA-RELEASE-CONSUMER-LOCAL.md), [`OA-RELEASE-CONSUMER-ALL-LOCAL`](evidence/OA-RELEASE-CONSUMER-ALL-LOCAL.md), and the current [hardening report](../hardening-report.md) | Publish signed immutable `v1.0.0` tags, resolve via the public proxy/checksum database, and exercise documented upgrades. Release owner; only the user authorizes release. |
