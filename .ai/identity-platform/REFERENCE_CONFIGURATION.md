...
| `struct:ref.phone.recovery` | `request_when_disabled = deny`; `complete_when_disabled = deny`; `proof = canonical_reset_capability_plus_eligible_independent_factor`; `sim_swap = negative_allow,positive_deny,unknown_deny,unavailable_deny`; `number_recycling = negative_allow,positive_deny,unknown_deny,unavailable_deny`; `carrier = negative_allow,positive_deny,unknown_deny,unavailable_deny`; `carrier_signal = required` | closed immutable risk/proof policy; `phone.recovery.enabled` is the sole enablement authority; enabling requires explicit operator acceptance of SIM-swap, number-recycling and carrier risks and never weakens the independent-factor requirement; absent required evidence is `unavailable` |
...
| `phone.recovery.policy` | closed policy `struct:ref.phone.recovery` | positive or unknown SIM-swap/number-recycling evidence and missing, unknown or unavailable required carrier evidence deny recovery; risk step-up cannot replace the eligible independent factor |
