...
The provider matrix also solely owns each provider's `oidc_logout` profile.
`identity/reference` MUST load that closed object with the selected immutable
provider profile; neither an application caller nor callback input may select,
replace, or synthesize an end-session endpoint, provider, issuer, session, or
post-logout target. `unsupported` and `unknown` both permit local session
revocation, but MUST NOT be represented as provider logout support. `unknown`
MUST remain blocked until every listed evidence-blocker ID is closed by pinned
official provider evidence and the required interoperability result. A
discovery-sourced endpoint MUST come only from the validated, issuer-bound,
pinned metadata used for that transaction. A fixed endpoint MUST exactly equal
the catalog value. Callback state is one-use and recovers provider, issuer,
endpoint, session/version, callback origin, and the allowlisted post-logout
target; response parameters never select any of those authorities.
...
| `struct:ref.oauth.provider_oidc_logout` | `status = supported,unsupported,unknown`; `end_session_endpoint_source = {kind,value}` where `kind = fixed,discovery,unsupported`; `state_correlation = required-single-use-exact,not-applicable-local-only`; `one_time_state_recovery = provider-and-session-derived-exclusively-from-bound-state,not-applicable-local-only`; `post_logout_redirect = allowlisted-local-redirect-after-provider-outcome,allowlisted-local-redirect-after-local-revocation`; `frontchannel_session_effect = local-only,provider-logout-attempt,unsupported,unknown`; `evidence_blocker = sorted_unique_array_of_registered_blocker_ids` | closed per-provider logout policy; `supported` requires a usable fixed or discovery endpoint and no blockers, `unsupported` rejects provider logout before provider I/O, `unknown` forbids a support claim and permits only a catalog-authorized best-effort attempt when its endpoint source resolves, and every disposition still completes authorized local revocation |
| `struct:ref.identity.privacy_export` | `format = identity-portable-json-v1`; `media_type = application/json`; `compression = none`; `states = queued,running,ready,failed,cancelled,expired`; `snapshot = immutable_contributor_set_plus_version_vector`; `postgres_snapshot = append_only_versioned_projection_or_transaction_staged_immutable_fragment`; `non_postgres_snapshot = exact_versioned_journal_checkpoint`; `sections = identity,accounts,identifiers,sessions,devices,organizations,memberships,consents`; `excluded = passwords,credential_verifiers,private_keys,recovery_material,session_bearers,provider_tokens,raw_provider_payloads,risk_signals,internal_audit_payloads`; `publication = all_required_contributors_then_identity_privacy_epoch_recheck`; `worker = bounded_one_job_generation_fenced_checkpointed`; `object_store = atomic_envelope_encrypted_write_only_publication`; `at_rest = envelope_aead`; `download = capability_authorized_server_decrypt_bounded_json_stream_with_whole_document_sha256`; `encrypted_bytes_to_caller = forbidden`; `deletion_or_anonymization = cancel_revoke_then_erase`; `legal_hold = limitation_manifest_only`; `unknown = fail_closed_reconcile` | sole portable asynchronous privacy-export policy; UTF-8 JSON uses schema version, RFC 3339 UTC timestamps, opaque IDs, stable section ordering, fragment and whole-artifact SHA-256 digests, and a manifest of included/omitted sections and provider-held limitations; reference readiness requires the exact contributor set, worker, encrypted object store, retained decryption key, capability issuer, and revocation/erasure consumer |
| `struct:ref.scim.authentication_schemes` | `maximum_entries = 8`; `unique_type = true`; `maximum_primary = 1`; `entry_1.type = oauthbearertoken`; `entry_1.name = OAuth Bearer Token`; `entry_1.description = Scoped bearer token`; `entry_1.spec_uri = https://www.rfc-editor.org/rfc/rfc6750`; `entry_1.documentation_uri = absent`; `entry_1.primary = true` | closed RFC 7643 authentication scheme catalog; every string is bounded by `scim.string_bytes`, only advertised schemes may authenticate SCIM requests, and runtime rejects a catalog that differs from accepted authentication |
| `struct:ref.scim.bulk_execution` | `admission = persist_parent_all_ordered_children_graph_and_scc_plan`; `matching_replay = same_connection_scope_and_exact_canonical_fingerprint`; `mismatch = conflict_before_mutation_without_prior_result`; `child_states = admitted,running,succeeded,failed,dependency_blocked,skipped`; `acyclic_singleton = one_child_one_transaction`; `cyclic_scc = one_bounded_all_or_nothing_transaction_with_deferred_within_scc_references`; `fail_on_errors_zero_or_absent = no_cutoff`; `cutoff = durable_failed_children_reach_positive_threshold`; `remaining_admitted_not_running = skipped`; `result_states = succeeded,failed`; `dependency_blocked = reconcile_without_wire_result`; `skipped_wire_operation = omitted`; `skipped_wire_status_location_version_error = absent`; `replay = processed_results_in_request_order_from_durable_checkpoints`; `audit = identity.scim.bulk_skip_child` | closed Bulk partial-execution policy; persisted-but-skipped is a durable unprocessed child state, not an RFC wire response status or failure, and no SCC transaction implies whole-request atomicity |
...
| `struct:ref.api_key.permission_reduction` | `default = narrow`; `allowed = narrow,revoke`; `selector = immutable_configuration_revision`; `narrow = persist_strict_current_subset_and_emit_api_key.permissions_changed.v1`; `revoke = persist_deny_all_and_revoked_then_emit_api_key.revoked.v1`; `version_fence = owner_authority_version+key_version+policy_version`; `unknown = deny_and_reconcile_original_command`; `caller_override = forbidden` | sole closed user-authority and organization-permission-ceiling reduction policy; the reference profile narrows to preserve still-authorized automation, while deployments may explicitly choose revoke for a configuration revision; neither choice may widen the issuance grant or permit removed authority before persistence |
...
| `scim.documentation_uri` | `REQUIRED URI` | absolute URI, 1..`scim.string_bytes` UTF-8 bytes; emitted unchanged as ServiceProviderConfig `documentationUri` |
| `scim.patch.enabled` | true | fixed reference capability; ServiceProviderConfig `patch.supported` is derived from this effective Boolean |
...
| `scim.filter.enabled` | true | fixed reference capability; ServiceProviderConfig `filter.supported` is derived from this effective Boolean |
| `scim.change_password.enabled` | false | fixed reference capability; SCIM password changes are rejected at the typed credential seam and ServiceProviderConfig advertises unsupported |
| `scim.sort.enabled` | true | fixed reference capability; ServiceProviderConfig `sort.supported` is derived from this effective Boolean |
| `scim.etag.enabled` | true | fixed reference capability; ServiceProviderConfig `etag.supported` is derived from this effective Boolean |
| `scim.authentication_schemes` | closed policy `struct:ref.scim.authentication_schemes` | exact bounded ServiceProviderConfig catalog and accepted runtime authentication; divergent advertisement or acceptance blocks readiness |
...
| `providers.<id>.oidc_logout` | exact `provider-matrix-v1` closed object | immutable catalog-owned status, endpoint source, one-use state recovery, redirect behavior, front-channel effect and evidence blockers; callers cannot override any member, and local logout remains available when provider logout is unsupported or unknown |
