...
| Multi-factor authentication | In | `identity/mfa`, `identity/mfa/postgres` | TOTP/OTP enrollment and challenge, recovery codes shown only at creation/full regeneration and never re-viewed, trusted devices, step-up, factor replacement/removal and lockout recovery. | Pinned TOTP vectors, plaintext re-view denial, PostgreSQL recovery/replay/race tests, complete HTTP enrollment and signin. |
...
| Native/provider-token social signin | In | `identity/oauth`, `identity/oauth/postgres`, `identity/oauth/providers`, `identity/http` | Accept only explicitly supported provider ID-token or opaque access-token profiles; bind issuer, audience/authorized party, nonce, platform client and subject; persist optional provider tokens safely; apply signup/linking policy and issue the normal session result. | Apple, Google, Facebook and LINE pinned fixtures plus current native/provider evidence, audience/nonce substitution tests, account collision tests and complete HTTP journey. |
...
| Capability | Scope | Owner(s) | Required acceptance | Verification boundary |
| --- | --- | --- | --- | --- |
| Admin | In | `identity`, `identity/session`, `identity/impersonation`, `authorization`, `identity/reference`, `identity/http` | Persist versioned platform-administrator roles and subject assignments through the authoritative authorization adapter; compose one-time bootstrap and later authorized assignment changes with identity state and immutable audit; list/search users, status/ban, role/permission decisions, delete/anonymize, credentials, sessions and impersonation. | Bootstrap/assignment transaction and recovery races, authorization matrix, denial/cross-tenant tests, deletion cascades, HTTP administration journey and audit inspection. |
...
| JavaScript project initialization and dependency upgrade CLI | Non-capability tooling | Framework scaffolding and package-manager mutation are not Go backend identity capabilities; migration, secret and diagnostic operations remain in scope through `identity/reference`. |
