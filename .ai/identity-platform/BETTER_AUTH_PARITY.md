...
| Multi-factor authentication | In | `identity/mfa`, `identity/mfa/postgres` | TOTP/OTP enrollment and challenge, recovery codes, trusted devices, step-up, factor replacement/removal and lockout recovery. | Pinned TOTP vectors, PostgreSQL recovery/replay/race tests, complete HTTP enrollment and signin. |
...
| Social OAuth / generic OAuth | In | `identity/oauth`, `identity/oauth/postgres`, `identity/oauth/providers` | Authorization code plus PKCE, state/nonce/redirect binding, encrypted durable token storage/refresh, verified linking/unlinking and configurable generic providers. | Protocol fixtures, PostgreSQL races/key rotation, hostile callbacks, and real/sandbox provider evidence for every declared built-in profile. |
...
| Capability | Scope | Owner(s) | Required acceptance | Verification boundary |
| --- | --- | --- | --- | --- |
| API keys | In | `identity/apikey`, `identity/apikey/postgres`, `identity/apikey/valkey` | User/organization-owned create, prefix lookup, list, rename, scopes, metadata, quota/refill, expiry, rotate and revoke; reveal secret once, digest at rest, and support declared database/secondary-storage fallback profiles. | Race/replay/redaction tests, PostgreSQL and Valkey quota/rotation/fallback proof, HTTP lifecycle. |
...
| JWT issuance and JWKS | In | `oauth-server/oidc`, `authentication/jwt` | Issue bounded signed JWTs for OAuth grants and an explicitly authorized authenticated-session exchange profile; publish/rotate JWKS and validate issuer/audience/subject/time/claims. The authentication primitive remains validation-only. | Independent verifier and relying-party tests, key-rotation overlap/compromise, session-exchange and OAuth token journeys. |
...
| Core HTTP rate limiting | In | `identity/http`, `identity/reference` | Default and per-route rules, trusted-client IP derivation, IPv4-mapped IPv6 normalization, configurable IPv6 subnet aggregation, Retry-After, atomic distributed storage and fail policy apply to client HTTP calls without weakening domain risk controls. | Route contract tests plus PostgreSQL/Valkey concurrency, proxy-spoofing, expiry and outage journeys. |
...
| Safe instrumentation | In | `identity/reference`, `telemetry` | Emit bounded metrics/traces/audit for lifecycle and provider outcomes with opt-in exporters, controlled cardinality and no secrets/PII; instrumentation failure cannot alter auth decisions. | Redaction/cardinality tests, exporter outage/shutdown and end-state trace correlation. |
...
| Operational tooling | In | `identity/reference` | Enumerate/generate selected schemas, plan/apply migrations, generate cryptographic configuration secrets and emit a bounded fully redacted diagnostic summary through public APIs; JavaScript project initialization and dependency upgrading are excluded. | Empty/populated migration rehearsals, entropy tests and golden redaction tests with secret-shaped inputs. |
...
| JavaScript project initialization and dependency upgrade CLI | Excluded | Framework scaffolding and package-manager mutation are not Go backend identity capabilities; migration, secret and diagnostic operations remain in scope through `identity/reference`. |
| Community plugin catalog | Excluded | Third-party catalog breadth is not an official backend capability; the typed extension-module contract remains in scope. |
...
| `two-factor` | Multi-factor authentication and two-factor security keys |
| `username` | Username |
...
HubSpot, Keycloak, Microsoft Entra ID, Okta, Patreon and Yandex; LINE and Slack
