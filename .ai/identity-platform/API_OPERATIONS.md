...
| `identity.sso.oidc-logout-complete` | `sso/oidc` / protocol | provider callback plus one-time logout state / origin | provider / once | Provider response and state; consumes state once, validates issuer/provider/session binding, and returns exactly one redirect, local-only, provider-complete, provider-error, timeout, or unknown-reconciliation outcome. |
...
| `identity.sso.saml-slo` | `sso/saml`, `identity/session` / protocol | signed SAML logout message with optional RelayState / origin | provider / once | Bounded LogoutRequest or LogoutResponse and issuer; RelayState is optional and, when present, is correlated and echoed exactly. |
