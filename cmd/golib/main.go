...
type module struct {
	...
	FamilyOrder         int             `json:"family_order,omitempty"`
...
func main() {
	if len(os.Args) < 2 {
		fatal("usage: golib <manifest|standalone-manifest|standalone-populate|standalone-release-prepare|standalone-clean-sums|standalone-proxy|validate|cohesion|documentation|specifications|assurance|select|safety>")
	}
	...
	case "standalone-release-prepare":
		if err := prepareStandaloneReleases(root, os.Args[2:]); err != nil {
			fatal("standalone release prepare: %v", err)
		}
...
func discover(root string) (catalog, error) {
	...
	cohesion, cohesionErr := loadCohesionConfig(root)
	if cohesionErr != nil {
		return catalog{}, cohesionErr
	}
	if cohesionErr := applyCohesionFamilies(&result, cohesion); cohesionErr != nil {
		return catalog{}, cohesionErr
	}
...
func catalogDocumentation(current catalog) map[string]string {
	...
	packageCatalog.WriteString("This consumer view contains independently releasable modules only. ")
	packageCatalog.WriteString("See the [engineering inventory](engineering-inventory.md) for tools, fixtures, and harnesses.\n\n")
	maxFamilyOrder := 0
	for _, item := range current.Modules {
		maxFamilyOrder = max(maxFamilyOrder, item.FamilyOrder)
	}
	for familyOrder := 1; familyOrder <= maxFamilyOrder; familyOrder++ {
		familyModules := []module{}
		for _, item := range current.Modules {
			if item.Releasable && item.FamilyOrder == familyOrder {
				familyModules = append(familyModules, item)
			}
		}
		if len(familyModules) == 0 {
			continue
		}
		family := familyModules[0]
		if familyOrder > 1 {
			packageCatalog.WriteString("\n")
		}
		fmt.Fprintf(&packageCatalog, "## %s\n\n%s\n\n", family.FamilyLabel, family.FamilyDescription)
		packageCatalog.WriteString("| Module | Kind | Lifecycle | Purpose | Owned dependencies | Services | Specifications |\n")
		packageCatalog.WriteString("| --- | --- | --- | --- | --- | --- | --- |\n")
		for _, item := range familyModules {
	...
	var engineering strings.Builder
	engineering.WriteString("# Engineering Inventory\n\n")
	engineering.WriteString("Generated by `go run ./cmd/golib manifest`; do not edit manually.\n\n")
	engineering.WriteString("This view contains every registered module, including repository tooling, fixtures, examples, interoperability harnesses, and benchmarks.\n\n")
	engineering.WriteString("| Module | Kind | Family | Lifecycle | Purpose | Owned dependencies | Services | Specifications |\n")
	engineering.WriteString("| --- | --- | --- | --- | --- | --- | --- | --- |\n")
	for _, item := range current.Modules {
		moduleLink := fmt.Sprintf("`%s`", item.Path)
		if item.Directory != "." {
			moduleLink = fmt.Sprintf("[`%s`](../%s)", item.Path, item.Directory)
		}
		fmt.Fprintf(
	...
	return map[string]string{
		"docs/engineering-inventory.md": engineering.String(),
