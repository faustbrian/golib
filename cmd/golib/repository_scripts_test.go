...
func TestModuleRunnerIsolatesServicesBetweenGates(t *testing.T) {
	root := testRepositoryRoot(t)
	repository := filepath.Join(t.TempDir(), "repository")
...
if [[ "$1" == "run" ]]; then
    printf 'pkg/example\n'
elif [[ "$1" == "env" && "$2" == "GOPROXY" ]]; then
    printf 'https://proxy.golang.org,direct\n'
elif [[ "$1" == "env" && "$2" == "GONOSUMDB" ]]; then
    printf '\n'
else
    exit 2
fi
`)
	writeFile(t, filepath.Join(repository, "scripts", "build-local-proxy.sh"), `#!/usr/bin/env bash
set -euo pipefail
mkdir -p "$1"
`)
	writeFile(t, filepath.Join(repository, "scripts", "start-services.sh"), `#!/usr/bin/env bash
set -euo pipefail
...
`)
	writeFile(t, filepath.Join(repository, "scripts", "stop-services.sh"), `#!/usr/bin/env bash
set -euo pipefail
printf 'stop:%s\n' "$(cat "$1")" >>"${TEST_STATE}/events"
`)
	writeFile(t, filepath.Join(repository, "scripts", "run-gate-with-evidence.sh"), `#!/usr/bin/env bash
set -euo pipefail
printf 'gate:%s:%s\n' "${SERVICE_GENERATION}" "$2" >>"${TEST_STATE}/events"
`)
	writeFile(t, filepath.Join(repository, "scripts", "audit-goals.sh"), `#!/usr/bin/env bash
set -euo pipefail
printf 'audit\n' >>"${TEST_STATE}/events"
`)
	for _, path := range []string{
		"go", "scripts/run-modules.sh", "scripts/build-local-proxy.sh",
		"scripts/start-services.sh", "scripts/stop-services.sh",
		"scripts/run-gate-with-evidence.sh", "scripts/audit-goals.sh",
	} {
	...
	command := exec.Command(filepath.Join(repository, "scripts", "run-modules.sh"), "check", "--modules", "pkg/example")
	command.Dir = repository
	environment := environmentWithValues(
		os.Environ(),
		"PATH", repository+string(os.PathListSeparator)+os.Getenv("PATH"),
	)
	environment = environmentWithValues(environment, "TEST_STATE", state)
	command.Env = environmentWithValues(environment, "GOLIB_VERIFICATION_SNAPSHOT", "1")
	if output, err := command.CombinedOutput(); err != nil {
		t.Fatalf("run module gates: %v\n%s", err, output)
	}
	events, err := os.ReadFile(filepath.Join(state, "events"))
	if err != nil {
		t.Fatal(err)
	}
	want := "start:1\ngate:1:test\nstop:1\nstart:2\ngate:2:race\nstop:2\naudit\n"
	if string(events) != want {
		t.Fatalf("service lifecycle events = %q, want %q", events, want)
	}
}
