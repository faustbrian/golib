...
`valkey.Store.Guard` returns the redacted lease-hash key plus exact owner and
token for a compatible atomic protected writer. It does not validate ownership
when constructed; the protected Valkey script must compare all three values at
the write boundary. The guard is valid only for the same deployment, prefix,
key derivation, and continuity epoch.
