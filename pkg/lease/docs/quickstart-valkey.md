...
For a reconstructible cache refresh on the same standalone deployment, derive
`guard, err := backend.Guard(owned)` and pass the guard to
`cache.Cache.SetIfOwned`. Do not log the guard or derive its storage key
yourself. See [protected writes](protected-writes.md).
