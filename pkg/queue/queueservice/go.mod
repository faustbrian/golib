module github.com/faustbrian/golib/pkg/queue/queueservice

go 1.26.5

require (
	github.com/faustbrian/golib/pkg/correlation v0.0.0-20260729185016-600a2ffaf74d
	github.com/faustbrian/golib/pkg/queue v0.0.0-20260809113453-4dd28d2c9e65
	github.com/faustbrian/golib/pkg/service v0.0.0-20260809100943-73e2977b1e90
	github.com/redis/go-redis/v9 v9.19.0
	go.opentelemetry.io/otel v1.44.0
	go.opentelemetry.io/otel/trace v1.44.0
)

require (
	github.com/appleboy/com v1.2.0 // indirect
	github.com/cespare/xxhash/v2 v2.3.0 // indirect
	github.com/faustbrian/golib/pkg/cli v0.0.0-20260729183302-ac9562ceb0b5 // indirect
	github.com/faustbrian/golib/pkg/identifier v0.0.0-20260729183302-ac9562ceb0b5 // indirect
	github.com/jackc/pgx/v5 v5.10.0 // indirect
	github.com/jpillora/backoff v1.0.0 // indirect
	github.com/valkey-io/valkey-go v1.0.76 // indirect
	go.uber.org/atomic v1.11.0 // indirect
	golang.org/x/sys v0.45.0 // indirect
)
