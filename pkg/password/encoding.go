...
func parseArgon2id(encoded string, limits Limits) (EncodedHash, error) {
	...
	read := func(field, prefix string, bitSize int) (uint64, error) {
		...
		value, err := strconv.ParseUint(number, 10, bitSize)
		if err != nil {
			return 0, ErrResourceRejected
		}
		return value, nil
	}
	m, e1 := read(params[0], "m=", 32)
	t, e2 := read(params[1], "t=", 32)
	p, e3 := read(params[2], "p=", 8)
	...
	parallelism := uint8(p)          //nolint:gosec // ParseUint above is explicitly limited to 8 bits.
