...
tooling behavior even when values round-trip. Consistent uppercase or lowercase
ULID text sorts in bytewise/C collation; mixed case is rejected and
locale-specific collations must be tested. `driver.Valuer` and `String` retain
canonical uppercase output; callers writing an established lowercase schema
must pass `StringLower()` explicitly.
