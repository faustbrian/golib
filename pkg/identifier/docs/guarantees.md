...
ULID parsing accepts consistently uppercase or lowercase 26-character text and
rejects mixed case. `String` returns canonical uppercase while `StringLower`
supports established lowercase schemas. The full 80-bit entropy field is
preserved. A generator starts a random sequence each
