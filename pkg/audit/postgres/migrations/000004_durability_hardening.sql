...
BEGIN
    IF pg_catalog.current_setting('server_encoding') <> 'UTF8' THEN
        RAISE EXCEPTION 'audit requires UTF8 database encoding'
            USING ERRCODE = '0A000';
    END IF;
END
$encoding_safety$;

SELECT pg_catalog.set_config('lock_timeout', '30s', true);
LOCK TABLE audit.records, audit.retention_events IN SHARE ROW EXCLUSIVE MODE;

DROP INDEX audit.retention_events_record_order_idx;
...
BEGIN
    IF jsonb_typeof(value) <> 'object'
       ...
       OR COALESCE(value ->> 'authentication_method', '') NOT IN (
           '', 'api_key', 'certificate', 'email_otp', 'hotp', 'mTLS',
           'magic_link', 'oauth2', 'oidc', 'passkey', 'password',
           'recovery_code', 'saml', 'session', 'sms_otp', 'totp',
           'webauthn', 'workload_identity'
       )
...
ALTER FUNCTION audit.prune_record(text, bytea)
    SET search_path = pg_catalog, audit, pg_temp;
ALTER FUNCTION audit.lock_retention_event()
    SET search_path = pg_catalog, audit, pg_temp;
ALTER FUNCTION audit.append_record(
    text, timestamptz, timestamptz, text, smallint, text, text, text, text,
    smallint, text, bytea, bytea
) SET search_path = pg_catalog, audit, pg_temp;
ALTER FUNCTION audit.capture_record_identity()
    SET search_path = pg_catalog, audit, pg_temp;
...
DO $audit_down$
BEGIN
    RAISE EXCEPTION 'audit durability hardening cannot be reversed because that could expose or erase accepted history'
        USING ERRCODE = '55000';
END
$audit_down$;
