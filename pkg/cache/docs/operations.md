...
not fenced by ordinary mutations; critical workflows should include a source
version in the value or key and reject obsolete writes at the source boundary.

For distributed cache refresh, use a Valkey lease and publish only through
`SetIfOwned`. The lease and cache adapters must point to the same standalone
Valkey deployment. Treat `ErrOwnershipLost` as a normal abandoned refresh, but
retain backend errors as operational failures. Never publish after releasing
the lease, and never reuse this cache-specific protocol for durable business
effects.
