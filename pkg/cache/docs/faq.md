...
distributed lock as cache functionality. A caller may acquire an external
Valkey lease and use `SetIfOwned` to prevent an expired loader from publishing,
but each service still performs its own miss and admission workflow.
