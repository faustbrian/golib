...
- optional fail-closed ownership-guarded publication for distributed refresh
  work, without presenting lease acquisition as cache functionality
...
- cache failures must not silently become misses unless policy explicitly says
  so
- ownership-guarded publication must atomically validate the active owner and
  token with the write and reject missing, expired, or successor ownership
