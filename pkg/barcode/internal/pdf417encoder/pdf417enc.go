...
// Macro describes a Macro PDF417 control block. FileID contains groups of
// three decimal digits, one group for each file-identification codeword.
type Macro struct {
	SegmentIndex int
	FileID       string
	LastSegment  bool
...
func (p *PDF417Encoder) GenerateBarcodeLogic(msg string, errorCorrectionLevel int) error {
	return p.GenerateBarcodeLogicWithControls(msg, errorCorrectionLevel, 0, nil)
}

// GenerateBarcodeLogicWithControls generates a symbol with an optional ECI
// prefix and Macro PDF417 control block.
func (p *PDF417Encoder) GenerateBarcodeLogicWithControls(
	msg string, errorCorrectionLevel, eci int, macro *Macro,
) error {
	...
	if eci != 0 {
		eciCodewords, eciErr := EncodeECI(eci)
		if eciErr != nil {
			return eciErr
		}
		highLevel = eciCodewords + highLevel
	}
	macroCodewords, err := encodeMacro(macro)
	if err != nil {
		return err
	}
	...
	ec, _ := GenerateErrorCorrection(dataCodewords, errorCorrectionLevel)
...
func encodeMacro(macro *Macro) (string, error) {
	if macro == nil {
		return "", nil
	}
	if macro.SegmentIndex < 0 || macro.SegmentIndex > 99_998 || macro.FileID == "" ||
		len(macro.FileID)%3 != 0 {
		return "", errors.New("invalid Macro PDF417 mandatory field")
	}
	...
	for offset := 0; offset < len(macro.FileID); offset += 3 {
		...
		result.WriteRune(rune(codeword)) //nolint:gosec // File ID codewords are bounded to 0 through 899.
	}
	if err := appendMacroText(&result, 0, macro.FileName); err != nil {
		return "", err
	}
	if err := appendMacroNumber(&result, 1, macro.SegmentCount); err != nil {
		return "", err
	}
	if err := appendMacroNumber(&result, 2, macro.Timestamp); err != nil {
		return "", err
	}
	if err := appendMacroText(&result, 3, macro.Sender); err != nil {
		return "", err
	}
	if err := appendMacroText(&result, 4, macro.Addressee); err != nil {
		return "", err
	}
	if err := appendMacroNumber(&result, 5, macro.FileSize); err != nil {
		return "", err
	}
	if err := appendMacroNumber(&result, 6, macro.Checksum); err != nil {
		return "", err
	}
	if macro.LastSegment {
		result.WriteRune(922)
	}

	return result.String(), nil
}

func appendMacroText(result *strings.Builder, field int, value string) error {
	if value == "" {
		return nil
	}
	encoded, err := EncodeHighLevel(value, CompactionText)
	if err != nil {
		return err
	}
...
func appendMacroNumber[T ~int | ~int64](result *strings.Builder, field int, value *T) error {
	if value == nil {
		return nil
	}
	if *value < 0 {
		return errors.New("negative Macro PDF417 numeric field")
	}
