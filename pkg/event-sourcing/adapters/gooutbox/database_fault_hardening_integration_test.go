...
`); err != nil {
	...
	prepareDeadlockOwner(t, ctx, first, "gooutbox-deadlock-first", 1)
	prepareDeadlockOwner(t, ctx, second, "gooutbox-deadlock-second", 2)
	...
	waitForDatabaseLock(t, ctx, pool, "gooutbox-deadlock-first")
...
`); err != nil {
		...
		if !errors.Is(stageErr, gooutbox.ErrOutboxWrite) ||
...
func databaseFaultStager(t *testing.T, tx pgx.Tx) *gooutbox.Stager {
	t.Helper()
	...
	codec, err := gooutbox.NewEnvelopeCodec(
		gooutbox.FixedTopic("database-fault-events"),
