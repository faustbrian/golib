...
func TestPrimaryFailoverRollsBackOpenCallerTransactionAsOnePair(t *testing.T) {
	...
	primary, err := tcpostgres.Run(
		...
		network.WithNetwork([]string{"gooutbox-primary"}, clusterNetwork),
	...
	const replicaData = "/tmp/gooutbox-replica"
	replica, err := testcontainers.Run(
		ctx,
		outboxPostgresIntegrationImage(t, "18"),
		network.WithNetwork([]string{"gooutbox-replica"}, clusterNetwork),
...
mkdir -p "$PGDATA"
rm -rf "$PGDATA"/*
printf '%s\n' 'gooutbox-primary:5432:*:gooutbox_replica:gooutbox_replica' > /tmp/gooutbox-replica.pgpass
chmod 600 /tmp/gooutbox-replica.pgpass
pg_basebackup --dbname='host=gooutbox-primary port=5432 user=gooutbox_replica passfile=/tmp/gooutbox-replica.pgpass sslmode=disable' --pgdata="$PGDATA" --format=plain --wal-method=stream --write-recovery-conf
chmod 700 "$PGDATA"
