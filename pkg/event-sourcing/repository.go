...
func (repository *AggregateRepository[ID, Aggregate]) reconstituteMessage(
	...
	logical, err := repository.upcasters.Upcast(input)
	if err != nil {
		return err
	}

	history := make([]HistoricalEvent, len(logical))
	for index, upcasted := range logical {
		decoded, err := repository.codec.Decode(upcasted.Event())
		if err != nil {
			return err
		}
		history[index], err = NewHistoricalEvent(HistoricalEventInput{
			SourceVersion: message.StreamVersion(),
			SegmentIndex:  uint32(index),
			SegmentCount:  uint32(len(logical)),
			Event:         decoded,
		})
		if err != nil {
			return fmt.Errorf("%w: invalid logical event", ErrCorruptHistory)
		}
	}
