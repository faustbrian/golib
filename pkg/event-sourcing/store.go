...
// GlobalReader provides optional store-wide ordered reads for replay and
// projections. Every returned message must have a global position.
...
// ReadGlobalOptions bounds one store-wide forward read.
//
// Its zero value is invalid.
type ReadGlobalOptions struct {
	fromPosition GlobalPosition
	toPosition   GlobalPosition
	limit        uint32
}

// ReadGlobalOptionsInput supplies one bounded inclusive global range.
//
// A zero ToPosition reads through the current end of the store.
type ReadGlobalOptionsInput struct {
	FromPosition GlobalPosition
	ToPosition   GlobalPosition
	Limit        uint32
}

// NewReadGlobalOptions validates one bounded inclusive global range.
func NewReadGlobalOptions(
	input ReadGlobalOptionsInput,
) (ReadGlobalOptions, error) {
	if input.FromPosition == 0 {
		return ReadGlobalOptions{}, invalid(
			"from_position",
			"must be greater than zero",
		)
	}
	if input.ToPosition != 0 && input.ToPosition < input.FromPosition {
		return ReadGlobalOptions{}, invalid(
			"to_position",
	...
	if input.Limit == 0 || input.Limit > MaxReadMessages {
		return ReadGlobalOptions{}, invalid(
			"limit",
			"must be within the read limit",
		)
	}

	return ReadGlobalOptions{
...
// FromPosition returns the inclusive first global position.
func (options ReadGlobalOptions) FromPosition() GlobalPosition {
	return options.fromPosition
}

// ToPosition returns the inclusive final global position, or zero for the
// current end.
func (options ReadGlobalOptions) ToPosition() GlobalPosition {
	return options.toPosition
}

// Limit returns the maximum number of messages in the read.
func (options ReadGlobalOptions) Limit() uint32 {
	return options.limit
}

// Valid reports whether the options are bounded and internally coherent.
func (options ReadGlobalOptions) Valid() bool {
	return options.fromPosition != 0 &&
