...
the library never starts a threshold worker or background goroutine. Encoder
and metadata-provider failures preserve their cause for `errors.Is` and
`errors.As` while redacting their diagnostic text. State-codec and
metadata-provider panics are contained as stable redacted error categories;
...
application codec composition, classified fallback, and blocking refresh are
implemented. Stale stored snapshots remain valid accelerators and are followed
by later history; `ErrSnapshotStale` describes a rejected store write rather
than a load-fallback category. Automatic threshold policy, schema-migration
helpers, and PostgreSQL storage remain planned and are not current package
guarantees.
