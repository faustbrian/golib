...
Package names below are design targets, not implemented APIs.
...
| `memory` | Conformant in-memory event and snapshot stores | Core only |
...
application aggregates
        |
        v
eventsourcing core <--- memory / eventtest / snapshot / projection
        ^                         / processmanager
        |
        +--- postgres
        +--- adapters/gokafka
...
The outbox adapter accepts one caller-owned PostgreSQL transaction and writes
event rows and outbox rows through that same transaction. It does not commit or
roll back the transaction. It uses the staging path, so no dispatcher can run
before the caller confirms commit. A direct event-store-to-Kafka dispatch is
