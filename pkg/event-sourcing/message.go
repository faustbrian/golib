...
// NewEventName validates a stable persisted event identity.
func NewEventName(value string) (EventName, error) {
	if !validName(value, MaxEventNameBytes) {
		return EventName{}, invalid(
			"event_name",
			"must be a non-empty canonical name",
		)
	}
...
// IsZero reports whether the event identity has not been assigned.
func (name EventName) IsZero() bool {
	return name.value == ""
}
...
func NewEncodedEvent(input EncodedEventInput) (EncodedEvent, error) {
	name, err := NewEventName(input.Name)
	if err != nil {
...
// EventName returns the stable persisted event identity without copying its
// payload.
func (message Message) EventName() EventName {
	return message.pending.event.Name()
}
