...
| `verkletree-bandersnatch-ipa-256-v0` | Package-owned experimental identity | Incremental pre-v1 implementation | Public immutable in-memory snapshots, canonical set/delete transitions, profile-bound roots, aggregate membership/non-membership proofs, and canonical capability-checked storage write batches over internal canonical topology, bounded vector commitments, strict encodings, and independent verifier reconstruction; no persisted reads, stable wire, witness, production, or Ethereum compatibility |
