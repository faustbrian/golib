...
| `verkletree-bandersnatch-ipa-256-v0` | Package-owned experimental identity | Incremental pre-v1 implementation | Structural metadata plus internal canonical topology, state, bounded vector commitments, complete mathematical roots, a strict package-owned root container, immutable root-bound batch transitions, and strict raw aggregate-opening payload decoding; no public tree, verified tree proof, witness, storage, external wire, production, or Ethereum compatibility |
