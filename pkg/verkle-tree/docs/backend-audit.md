...
`banderwagon` crate manifest also omits its license expression even though the
repository root carries pinned MIT and Apache-2.0 license files. This graph is
accepted only as a reproducible research generator and is rejected as a
