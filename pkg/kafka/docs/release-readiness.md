...
| KAF-F004 | High | PLAIN credential replacement is proven only through a bounded broker restart. Kafka 4.3.1's exercised default JAAS verifier does not reload in place, so zero-downtime multi-broker credential rotation is not established. Treating provider refresh alone as proof would expose rolling deployments to authentication failure. | Open for the production rotation objective. Current documentation explicitly withholds the zero-downtime claim. | Prove an overlap-first multi-broker rotation using a reviewed reloadable server callback or overlapping principals, including old-credential rejection, uninterrupted acknowledged delivery, reconnect bounds, and redaction. |
