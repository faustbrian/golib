...
| Mutation tool | patched Gremlins v0.6.0 | 2,497 of 2,497 viable root-package mutants and 49 of 49 bundled slog-adapter mutants killed; 100% efficacy and mutator coverage on 2026-07-27 |
...
| Apache Kafka | Minimum reviewed release and current 4.3.1, KRaft, three brokers | Current 4.3.1 executes as three combined KRaft broker/controller nodes with exact runtime assertion. RF=3, `min.insync.replicas=2`, clean leader election, continued acks-all delivery at ISR=2, endpoint and ISR recovery, committed/aborted producer transactions before and after broker-process recovery, same-transactional-ID producer fencing, broker-enforced transaction expiry, consume-transform-produce recovery after terminating a real child process between output acknowledgement and commit, and two-process eager plus cooperative rebalance abort and redelivery pass locally. Minimum-version, separated-role, secured, response-loss unknown-outcome, broader fault, and managed-service evidence remains unverified. |
...
| Producer | Single, batch, async, ordering, failure, shutdown | Policy APIs and deterministic tests exist. The pinned single-broker fixture proves ordered batch and asynchronous delivery metadata, exact broker-visible values, graceful shutdown draining an admitted asynchronous record, rejection after shutdown, and successful delivery under a 1 KiB/s client-ID quota with a positive post-response throttle event. Throttling remains request-level because Kafka produce responses can cover many records. The Apache fixture proves explicit-partition synchronous delivery before failure, after clean leader failover at ISR=2, and after ISR=3 recovery; ambiguous-outcome broker evidence remains unverified. |
