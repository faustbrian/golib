...
| Performance | Equivalent producer, consumer, transaction, replay, inspection, reconnect, resource, and policy-overhead workloads | The isolated client harness has separate real-broker single-record, batch, bounded asynchronous, and eight-partition correctness evidence. Timed evidence covers equivalent idempotent all-ISR synchronous single-record, 10/100-record batch, bounded 10/100-record asynchronous-window, and balanced Murmur2-keyed plus explicitly assigned 80-record eight-partition contracts for the package policy, raw franz-go, and Sarama. Each uses keying, payload, and compression variants, 20 samples of 50 operations, raw results, per-workload environment fingerprints, and variance-preserving analysis. Kafka-go is an unranked single-partition correctness control because v0.4.51 cannot match idempotence. Automatic unkeyed multi-partition production is excluded because franz-go and Sarama do not expose equivalent distribution strategies. The remaining workload and controlled-environment matrix is unverified. |
