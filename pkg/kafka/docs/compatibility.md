...
| Performance | Equivalent producer, consumer, transaction, replay, inspection, reconnect, resource, and policy-overhead workloads | The isolated client harness has separate real-broker single-record, batch, and bounded asynchronous correctness evidence for the package policy, raw franz-go, Sarama, and an unranked kafka-go control. Timed evidence covers equivalent idempotent all-ISR synchronous single-record, 10/100-record batch, and bounded 10/100-record asynchronous-window contracts for the first three clients on one local partition, with keying, payload, and compression variants, 20 samples of 50 operations, raw results, environment fingerprints, and variance-preserving analysis. Kafka-go production is excluded because v0.4.51 cannot match idempotence. The remaining workload and controlled-environment matrix is unverified. |
