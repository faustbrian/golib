...
| Apache Kafka | Minimum reviewed release and current 4.3.1, KRaft, three brokers | Current 4.3.1 executes as three combined KRaft broker/controller nodes with exact runtime assertion. RF=3, `min.insync.replicas=2`, clean leader election, continued acks-all delivery at ISR=2, endpoint and ISR recovery, committed/aborted producer transactions before and after broker-process recovery, same-transactional-ID producer fencing, consume-transform-produce recovery after terminating a real child process between output acknowledgement and commit, and two-process eager plus cooperative rebalance abort and redelivery pass locally. Minimum-version, separated-role, secured, transaction-timeout, unknown-outcome, broader fault, and managed-service evidence remains unverified. |
