...
| Replay | Offset/timestamp planning, exact ranges, gaps, resume | Local and broker-validated checkpoint-aware dry-run plans, exact-partition timestamp-window plans, explicit side-effect opt-in, broker-boundary preflight, exact no-reset ranges, independently bounded fetch and cross-partition handler concurrency, resumable per-range progress, cancellation, and bounded shutdown are implemented. The single-node fixture proves an executable timestamp-derived plan, broker-validated planning without consuming execution, interrupted replay, external-checkpoint resume, bounded cross-partition handler overlap with sequential per-partition order, and out-of-range rejection; real-broker cancellation and broker gap/truncation/compaction evidence remain incomplete |
...
| Observability | Producer, consumer, rebalance, transaction, replay, inspection, and lifecycle events plus optional adapters | Root payload-free producer delivery, nontransactional consumer record/batch/commit/poll and group lifecycle, producer and consume-transform-produce transaction lifecycle, and producer/consumer/transaction-processor broker events are deterministically tested. The single-broker fixture proves producer and consume-transform-produce begin/commit/abort events plus transaction-processor broker requests. The independently versioned OpenTelemetry adapter maps every current event, pins development-status messaging semantic conventions 1.43.0, defaults every identity attribute off, and has exact local statement, fuzz, race, and allocation evidence. Standalone authentication, complete rebalance timing, replay, inspection, health/shutdown lifecycle, cross-message propagation, and slog coverage remains unimplemented. |
