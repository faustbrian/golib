...
| Replay | Offset/timestamp planning, exact ranges, gaps, resume | Local and broker-validated checkpoint-aware dry-run plans, explicit side-effect opt-in, broker-boundary preflight, exact no-reset ranges, resumable per-range progress, cancellation, and bounded shutdown are implemented. The single-node fixture proves broker-validated planning without consuming execution, interrupted replay, external-checkpoint resume, and out-of-range rejection; timestamp planning and broker gap/truncation/compaction evidence remain unverified |
