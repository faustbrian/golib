...
| Root observation policy | Expose copied Kafka policy events through bounded synchronous callbacks. | Producer delivery, consumer processing/commit/group lifecycle, producer and consume-transform-produce transaction lifecycle, replay plan/record/run/shutdown lifecycle, and producer/consumer/transaction-processor/replay broker events contain no payloads, credentials, endpoints, arbitrary headers, or franz-go types; broader event coverage remains planned. | Delivery, nontransactional consumption, group, transaction, replay lifecycle, and broker activity implemented |
