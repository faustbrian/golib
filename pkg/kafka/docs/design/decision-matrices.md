...
| Root observation policy | Expose copied Kafka policy events through bounded synchronous callbacks. | Producer delivery and consumer processing/commit events contain no payloads, credentials, arbitrary headers, or franz-go types; broader event coverage remains planned. | Producer delivery and nontransactional consumer processing implemented |
