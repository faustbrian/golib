...
| Root observation policy | Expose copied Kafka policy events through bounded synchronous callbacks. | Producer delivery, consumer processing/commit, and producer/consumer broker connection/request/throttle/disconnect events contain no payloads, credentials, endpoints, arbitrary headers, or franz-go types; broader event coverage remains planned. | Producer delivery, nontransactional consumer processing, and producer/consumer broker activity implemented |
