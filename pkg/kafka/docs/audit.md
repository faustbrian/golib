...
| Hooks and telemetry | An optional root `ObserverPolicy` reports copied, payload-free synchronous producer delivery plus consumer record, batch, commit, and poll completion events in registration order. It bounds callback count and a shared cooperative deadline, contains and reports observer errors and panics, and fences same-client mutating and lifecycle reentrancy. | Add connection, authentication, broker request/throttle, retry, rebalance, transaction, replay, inspection, health, and shutdown events; add optional OpenTelemetry and slog adapters. |
| Evidence | Unit, race, exact statement and mutation coverage, nine fuzz targets, docs, and microbenchmarks pass. One fixture uses one Confluent Local 7.5.0 broker. A second fixture asserts Apache Kafka 4.3.1 at runtime across three combined KRaft broker/controller nodes and proves one broker-process failure/recovery path for inspection, production, and producer transactions. | Add secured brokers, authentication failures, multiprocess rebalances, transactional fencing/unknown outcomes, replay faults, stress/leaks, adapters, clean consumer, broader compatibility, and equivalent-client benchmarks. |
