...
| Evidence | Unit, race, exact statement and mutation coverage, five fuzz targets, docs, and microbenchmarks pass. One integration test uses one Confluent Local 7.5.0 broker. | Add multi-broker Apache Kafka, failures, auth, transactions, multiprocess rebalances, replay faults, stress/leaks, adapters, clean consumer, compatibility, and equivalent-client benchmarks. |
