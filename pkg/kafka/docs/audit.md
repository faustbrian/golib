...
| Inspection and health | Explicit bounded calls expose cluster identity, controller visibility, sorted brokers, exact replica/ISR/offline sets, leader epochs, beginning/end offsets, effective `min.insync.replicas`, and group lag. All calls derive an owned request timeout and fail closed on incomplete, inconsistent, excessive, unauthorized, or unavailable state. The pinned single-broker fixture proves cluster, controller, topic durability, and offset inspection. `Health` remains only a dependency ping. | Add group member assignments, topic retention/compaction configuration, typed multi-target partial results, transaction/producer diagnostics, multi-broker failure evidence, and distinct stateful liveness/readiness/dependency/diagnostic policy. |
...
| Evidence | Unit, race, exact statement and mutation coverage, six fuzz targets, docs, and microbenchmarks pass. One integration test uses one Confluent Local 7.5.0 broker. | Add multi-broker Apache Kafka, failures, auth, transactions, multiprocess rebalances, replay faults, stress/leaks, adapters, clean consumer, compatibility, and equivalent-client benchmarks. |
