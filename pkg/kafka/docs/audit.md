...
| Consumer groups | A classic cooperative-sticky group polls up to a record count, bounds concurrent fetches plus aggregate and per-partition fetch bytes, runs one record at a time, stops each partition at its first failure, and commits only contiguous successful prefixes. Independent partitions continue. Rebalances are blocked during the poll. | Add explicit generation ownership, assignment/revocation/loss handling, batch handlers, parallel partitions, pause/drain/shutdown, retry and dead-letter strategies, lifecycle observations, and a policy for decompressed or broker-oversized batches. |
...
The current tree subsequently killed all 921 viable mutants with 100% test
