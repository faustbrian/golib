...
| Hooks and telemetry | An optional root `ObserverPolicy` reports copied, payload-free synchronous record, batch, and asynchronous producer completion events in registration order. It bounds callback count and a shared cooperative deadline, contains and reports observer errors and panics, and fences same-producer reentrancy. | Add connection, authentication, broker request/throttle, consumer, rebalance, commit, transaction, replay, inspection, health, and shutdown events; add optional OpenTelemetry and slog adapters. |
