...
| Producer | Synchronous single, synchronous batch, and bounded asynchronous methods return per-record delivery metadata. Keyed production is the safe default; exact partitions require an explicit record policy; detected idempotent-producer data loss stops the producer; delivery errors include redacted fatal and ambiguous classifications; drain, abort, and shutdown are explicit. | Add byte-level client buffering, broker-throttling metadata, observer callbacks, and real-broker failure and shutdown evidence. |
...
The current tree subsequently killed all 854 viable mutants with 100% test
