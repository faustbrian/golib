...
func TestSearchUsesPITSearchAfterAndSignedQueryBoundCursor(t *testing.T) {
	...
	client, err := adapter.New(adapter.Config{
		...
		Search: &adapter.SearchConfig{
			...
			Resolver: adapter.IndexResolverFunc(func(_ context.Context, tenant, logical string, access adapter.IndexAccess) (adapter.IndexTarget, error) {
				...
				return adapter.IndexTarget{Name: "tenant-a-locations-v3", Fingerprint: "mapping-v3-fingerprint"}, nil
			}),
		},
	})
	if err != nil {
	...
	request := search.Request{
		...
		Sort:         []search.Sort{{Field: "population", Direction: search.Descending}, {Field: "_id", Direction: search.Ascending}},
...
func TestSearchTranslatesTypedCapabilitiesWithoutCrossIndexLeakage(t *testing.T) {
	...
	server := httptest.NewTLSServer(http.HandlerFunc(func(writer http.ResponseWriter, request *http.Request) {
		...
		writer.Header().Set("Content-Type", "application/json")
		_, _ = io.WriteString(writer, `{"took":2,"timed_out":false,"_shards":{"total":2,"successful":1,"skipped":0,"failed":1,"failures":[{"index":"private-index","reason":{"type":"query_shard_exception","reason":"sensitive query"}}]},"hits":{"total":{"value":0,"relation":"eq"},"hits":[]},"aggregations":{"ranges":{"buckets":[]}},"suggest":{"names":[{"text":"hel","offset":0,"length":3,"options":[]}]}}`)
	}))
	t.Cleanup(server.Close)
	client := newSearchClient(t, server, time.Now)
...
func TestSearchSurfacesFinalPITCleanupFailure(t *testing.T) {
	...
	t.Cleanup(server.Close)
	client := newSearchClient(t, server, time.Now)
	_, err := client.Search(t.Context(), search.Request{Tenant: "tenant-a", Index: "locations", Query: search.MatchAllQuery{}, Sort: []search.Sort{{Field: "_id", Direction: search.Ascending}}, Page: search.CursorPage{Size: 10, KeepAlive: time.Minute}})
...
func newSearchClient(t *testing.T, server *httptest.Server, clock func() time.Time) *adapter.Client {
	...
	client, err := adapter.New(adapter.Config{
		...
		Search: &adapter.SearchConfig{Limits: search.DefaultLimits(), CursorCodec: codec, Clock: clock, LocaleAnalyzers: map[string]string{"fi": "finnish"},
			Resolver: adapter.IndexResolverFunc(func(context.Context, string, string, adapter.IndexAccess) (adapter.IndexTarget, error) {
				return adapter.IndexTarget{Name: "tenant-a-locations-v3", Fingerprint: "mapping-v3"}, nil
			}),
		},
	})
	if err != nil {
