...
for image in "$kafka" "$confluent" "$maven"; do
	...
	printf '%s' "$manifest" | grep -F -- "$digest" >/dev/null || {
...
grep -F -- "$kafka" providers/confluent/scripts/check-confluent.sh >/dev/null
grep -F -- "$confluent" providers/confluent/scripts/check-confluent.sh >/dev/null
grep -F -- "$maven" providers/glue/scripts/check-wire-reference.sh >/dev/null
grep -F -- "$maven" providers/confluent/scripts/check-wire-reference.sh >/dev/null
grep -F -- 'kafka-schema-serializer' providers/confluent/testdata/reference/pom.xml >/dev/null
grep -F -- '<version>8.3.1</version>' providers/confluent/testdata/reference/pom.xml >/dev/null
grep -F -- 'b9cb84802804bb7feff2bc998b386ccd3e6b1a2342d168b21e792cfb62dabdcb' \
	providers/confluent/scripts/check-wire-reference.sh >/dev/null
grep -F -- 'cc7a225a7ae24ad4b62488e7c75480a2a9470e4282a83281934b9a2e3ea196d1' \
	providers/confluent/scripts/check-wire-reference.sh >/dev/null
grep -F -- 'schema-registry-serde' providers/glue/testdata/reference/pom.xml >/dev/null
grep -F -- '<version>1.1.27</version>' providers/glue/testdata/reference/pom.xml >/dev/null
