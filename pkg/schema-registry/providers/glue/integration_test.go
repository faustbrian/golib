...
var errIntegrationWouldRegister = errors.New("integration test refused to register a new schema version")
...
func (readOnlyGlue) RegisterSchemaVersion(context.Context, *awsglue.RegisterSchemaVersionInput, ...func(*awsglue.Options)) (*awsglue.RegisterSchemaVersionOutput, error) {
	return nil, errIntegrationWouldRegister
}
...
func TestProviderAgainstAWSGlueService(t *testing.T) {
	...
	canonicalizer := registryavro.New(170_000)
	provider, err := registryglue.New(registryglue.Config{
		API:            readOnlyGlue{client: client},
		Scope:          region + ":" + registry,
	...
	if err != nil {
		if errors.Is(err, errIntegrationWouldRegister) {
