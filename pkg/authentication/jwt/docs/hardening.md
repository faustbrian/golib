...
| `ES256K` | secp256k1 public key | Accepted only when the pinned JWX build includes its `jwx_es256k` support. |
...
contains `verify`. Asymmetric private keys, `none`, unknown or deprecated
algorithms, algorithm/key-type confusion, weak HMAC keys, wrong EC curves, and
RSA moduli outside the work bound are invalid configuration. The validator
does not infer an algorithm from a key.
...
| Identity | Non-empty exact issuer, configured audience, non-empty subject, issued-at time, and expiration. | Missing or malformed required claims, issuer/audience mismatch, future `iat`, expired token, or premature `nbf`. |
...
| Concurrent refresh | One in-process fetch is shared by overlapping callers. At most 128 provider operations are admitted. |
...
- RFC 7520 and the complete supported algorithm/key matrix:
