...
.PHONY: actionlint api-compat api-update architecture benchmark check check-all \
	coverage docs examples format format-check fuzz lint mutation nilaway race \
	provenance staticcheck test tidy-check consumer-compat vet vuln
...
actionlint:
	$(ACTIONLINT) ../.github/workflows/math-ci.yml
...
check: tidy-check format-check vet architecture provenance test race coverage fuzz \
	mutation examples consumer-compat benchmark docs api-compat actionlint lint \
	staticcheck vuln
