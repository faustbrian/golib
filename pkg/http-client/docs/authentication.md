...
## Caller-owned token caching and invalidation

`NewCachedTokenSource` adds coordinated refresh and explicit invalidation to a
provider-specific `ContextTokenSource` without constraining how that provider
obtains tokens:

```go
source, err := httpclient.NewCachedTokenSource(httpclient.TokenCacheOptions{
	Client: client,
...
The caller remains responsible for applying the token to the provider request
and classifying the provider response. When a provider rejects the exact token
used by a request, invalidate that observed value before the one approved
retry:

```go
if response.StatusCode == http.StatusUnauthorized &&
	source.Invalidate(token.AccessToken) {
	// Retry once through the caller-owned provider policy.
}
```

Invalidation uses exact comparison and returns false for empty, stale, newer,
or already-invalidated tokens. This prevents one concurrent rejected request
from discarding a token another request has already refreshed. The cache does
not decide retry count, HTTP method safety, provider error semantics, or token
request encoding.

One caller performs a refresh while others wait cancelably. Closing the owning
`Client` cancels refreshes and rejects cached tokens. Returned token values are
independent copies. Upstream failures are wrapped in `TokenCacheError`, whose
message does not render its cause or token material.
