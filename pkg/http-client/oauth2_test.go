...
func TestCachedTokenSourceInvalidatesOnlyTheObservedToken(t *testing.T) {
	t.Parallel()
	...
	var calls atomic.Int64
	upstream := ContextTokenSourceFunc(func(context.Context) (*oauth2.Token, error) {
		call := calls.Add(1)
	...
	if calls.Load() != 1 || first.AccessToken != "token-1" || second.AccessToken != "token-1" {
		t.Fatalf("cached tokens = %#v, %#v, calls %d", first, second, calls.Load())
	}
	first.AccessToken = "caller-mutated"
	if second.AccessToken != "token-1" {
		t.Fatal("cached token callers share mutable state")
	}
	if source.Invalidate("") || source.Invalidate("different-token") {
		t.Fatal("unobserved token invalidated the cache")
	}
	if !source.Invalidate("token-1") || source.Invalidate("token-1") {
		t.Fatal("observed token invalidation result changed")
	}
	refreshed, err := source.Token(context.Background())
	if err != nil {
		t.Fatalf("refreshed token: %v", err)
	}
	if calls.Load() != 2 || refreshed.AccessToken != "token-2" {
		t.Fatalf("refreshed token = %#v, calls %d", refreshed, calls.Load())
	}
}

func TestClientCredentialsTokenSourceSupportsObservedTokenInvalidation(t *testing.T) {
	t.Parallel()
	...
	if !source.Invalidate(token.AccessToken) {
		t.Fatal("client-credentials token was not invalidated")
	}
}

func TestCachedTokenSourceCoordinatesCancelableWaiters(t *testing.T) {
	t.Parallel()
...
func TestCachedTokenSourceRejectsInvalidPolicyAndRefreshResults(t *testing.T) {
	t.Parallel()
	...
	var typedNilSource *oauth2NilContextTokenSource
	for _, options := range []TokenCacheOptions{
		{Source: validSource},
		{Client: client},
		{Client: client, Source: typedNilSource},
		{Client: client, Source: validSource, EarlyExpiry: -time.Second},
	} {
		if _, err := NewCachedTokenSource(options); !errors.Is(err, ErrInvalidAuthentication) {
	...
	if _, err := NewCachedTokenSource(TokenCacheOptions{
		Client: closedClient, Source: validSource,
	}); !errors.Is(err, ErrInvalidAuthentication) {
		t.Fatalf("closed client cache error = %v", err)
	}

	fixedNow := time.Unix(1_700_000_000, 0)
	source, err := NewCachedTokenSource(TokenCacheOptions{
		Client: client, Source: validSource, EarlyExpiry: time.Minute,
		Now: func() time.Time { return fixedNow },
	})
	if err != nil {
		t.Fatalf("construct explicit cache: %v", err)
	}
	if !source.now().Equal(fixedNow) || source.earlyExpiry != time.Minute {
		t.Fatalf("explicit cache policy = %#v", source)
	}
	var nilContext context.Context
	if _, err := source.Token(nilContext); !errors.Is(err, ErrInvalidAuthentication) {
		t.Fatalf("nil token context error = %v", err)
	}
	canceled, cancel := context.WithCancel(context.Background())
	cancel()
	if _, err := source.Token(canceled); !errors.Is(err, context.Canceled) {
		t.Fatalf("pre-canceled token context error = %v", err)
	}

	secret := errors.New("do-not-render")
	for _, test := range []struct {
		...
		},
		{
			...
			want: ErrInvalidOAuth2Token,
		},
		{
			name: "unsafe token",
			source: ContextTokenSourceFunc(func(context.Context) (*oauth2.Token, error) {
				return &oauth2.Token{AccessToken: "unsafe\r\ndo-not-render", TokenType: "Bearer"}, nil
			}),
			want: ErrInvalidOAuth2Token,
		},
	} {
		t.Run(test.name, func(t *testing.T) {
			cached, err := NewCachedTokenSource(TokenCacheOptions{
				Client: client, Source: test.source,
			})
			if err != nil {
				t.Fatalf("construct cache: %v", err)
			}
			_, err = cached.Token(context.Background())
			var cacheError *TokenCacheError
			if !errors.As(err, &cacheError) || !errors.Is(err, test.want) {
				t.Fatalf("refresh error = %#v", err)
			}
			if strings.Contains(err.Error(), "do-not-render") {
				t.Fatalf("refresh error rendered secret: %q", err)
			}
		})
	}
}

func TestCachedTokenSourceStopsLeaderAndWaiterWhenClientCloses(t *testing.T) {
	t.Parallel()
...
func TestCachedTokenSourcePrefersCallerCancellationAfterRefresh(t *testing.T) {
	t.Parallel()
	...
	var cacheError *TokenCacheError
	if !errors.As(err, &cacheError) || !errors.Is(err, context.Canceled) {
		t.Fatalf("post-refresh cancellation error = %v", err)
	}
}
