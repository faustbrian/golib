...
## Can a concurrency lease bound outbound provider work?

Yes, at the application-operation boundary. The caller may acquire one
cluster-wide lease before a provider operation and release it after the complete
operation returns. `http-client` still owns outbound request pacing, retries,
and transport policy. Do not acquire and release around individual retry
attempts because that would let one logical operation evade the concurrency
bound.
