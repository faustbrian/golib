...
func TestInteractiveRequiresEachRuntimeDependency(t *testing.T) {
	...
	for name, omit := range map[string]func(*Execution){
		"events":   func(execution *Execution) { execution.Events = nil },
		"terminal": func(execution *Execution) { execution.Terminal = nil },
		"output":   func(execution *Execution) { execution.Output = nil },
	} {
		t.Run(name, func(t *testing.T) {
			t.Parallel()

			terminal := NewVirtualTerminal(80, 24)
			execution := Execution{Output: terminal, Events: terminal, Terminal: terminal}
			omit(&execution)
			_, err := runInteractive(context.Background(), prompt, execution)
			if !errors.Is(err, ErrTerminalUnavailable) || terminal.Acquired() {
				t.Fatalf("runInteractive() error = %v, acquired = %t", err, terminal.Acquired())
			}
		})
	}
}
...
func TestInteractiveIgnoresReplayFromAnotherPromptKind(t *testing.T) {
	...
	prompt := Prompt[string]{definition: definition[string]{
		id: "name", label: "Name", kind: KindText,
		retry: RetryPolicy{MaxAttempts: 1},
		parse: func(value string) (string, error) { return value, nil },
	}}
	value, err := runInteractive(ctx, prompt, Execution{
...
func TestInteractiveTextRejectsMultilineReplayAndPaste(t *testing.T) {
	...
	ctx := context.WithValue(context.Background(), formNavigationContextKey{}, interaction)
	value, err := runInteractive(ctx, prompt, Execution{
		Output: replayTerminal, Events: replayTerminal, Terminal: replayTerminal,
		Capabilities: Capabilities{InputTerminal: true, OutputTerminal: true},
	})
	if err != nil || value != "fresh" {
	...
	_, err = runInteractive(context.Background(), prompt, Execution{
		Output: pasteTerminal, Events: pasteTerminal, Terminal: pasteTerminal,
		Capabilities: Capabilities{InputTerminal: true, OutputTerminal: true},
	})
...
func TestInteractiveSecretEmptyStateAndCapabilityBoundaries(t *testing.T) {
	...
	terminal := NewVirtualTerminal(80, 24)
	if err := writeInteractive(Execution{Output: terminal}, definition[string]{
		id: "token", label: "Token", secret: SecretToken,
	}, "", "", 80); err != nil {
		t.Fatal(err)
	}
	...
	capabilities := Capabilities{
		InputTerminal: true, OutputTerminal: true, Color: ColorTrueColor,
	}
