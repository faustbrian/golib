...
| GJ-007 | Medium | Named parameter decoding inherited duplicate-name behavior inside nested params objects. | `DecodeParams` | **Fixed.** Iterative unique-name validation now covers every object depth before exact-name and typed decoding; covered by `TestDecodeParams`. |
